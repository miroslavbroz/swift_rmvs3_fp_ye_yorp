...
c   both filters A and B can have different dimensions now
      ...
      subroutine io_init_filter_2ND(infile,nbod,ntp,dtfilter_param,
     :  outfile,iflgchk)
...
c Filter parameters are given to io_write_filter_2ND.f in /filter_2ND/ common block
...
c  read input parameters
      ...
      if (ierr.ne.0) then
        write(*,*) 'io_init_filter_2ND: Error opening file ',infile,'.'
        call util_exit(1)
      endif
...
20      format('io_init_filter_2ND: Warning: DOS-like EOLs in ', a)
      endif
...
c  chelck PL id
      if ((filter_plid.gt.-2).or.(filter_plid.lt.-FMAXNPL)) then
        write(*,*) 'io_init_filter_2ND: Error: filter_plid is outside',
     &    ' interval [-FMAXNPL, -2].'
        call util_exit(1)
      endif
