...
c                                          (character*80)
c
c Other filtration parameters in common block /filter_2ND/!
c
...
c - flt_cb.inc include file with /filter_2ND/ common block
...
c - sigma was multiplied by q (due to Trojans)
      ...
      subroutine io_write_filter_2ND(time,nbod,ntp,mass,xh,yh,zh,vxh,
...
c
c  orbital elements are stored in common block /elements_2ND/, will be
...
c  do this block only once
        if (i2nd.eq.0) then
          if (filter_write) then
            ...
            if(ierr.ne.0) then
              write(*,*) ' SWIFT ERROR: in io_write_filter_2ND: '
...
c  calculate orbital elements for planets and test particles
      ...
      upflg_osc = .true.        ! update flag for io_write_proper_2ND.f subroutine
      FN(1)=FN(1)+1
      gm=mass(1)
c      write(*,*) 'io_write_filter_2ND: time = ', time, ' FN = ', FN(1),
c     :  FN(2), FN(3), FN(4)
...
c
      if (FN(filter_N).ge.FBUFN) then
        ...
        n=filter_N
        if (filter_stat(n:n).eq.'A') then
          call filter_elmts_write_2ND(iu,elmts,n,nbod,ntp,istat,
